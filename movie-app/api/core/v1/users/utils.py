...
from api.core.v1.users.models import UserCreate
...
def populate_users(session: Session, qty: int, users_limit: int = 50):
    users = session.exec(select(User)).all()
    if not len(users) > users_limit:
        generated_users = [generate_user() for x in range(0, qty + 1)]
...
def init_db_users() -> None:
    with Session(engine) as session:
        ...
        populate_users(session=session, qty=50)
